...
const runBot = async (
  ...
  return new Promise<Result>((resolve) => {
    const timer = setTimeout(() => {
      const inv = bot.inventory?.slots?.filter((s) => s && s.count > 0).map((s) => `${s!.name}x${s!.count}`).join(", ") ?? "";
      try { bot.end(); } catch {}
      resolve({ name, elapsed: Math.round(timeoutMs / 1000), success: false, message: "timeout", inventory: inv });
    }, timeoutMs);
    ...
    bot.once("spawn", async () => {
      try {
        ...
        await new Promise<void>((res) => {
          const check = () => { if (bot.inventory?.slots) res(); else setTimeout(check, 100); };
          check();
        });
        await sleep(1000);
        ...
        if (stepId.startsWith("mine")) {
          ...
          if (pickSlot >= 0) {
            ...
            console.log(`[${name}] pickaxe in slot ${pickSlot}, held: ${bot.heldItem?.name ?? "null"}`);
          } else {
            console.log(`[${name}] NO PICKAXE FOUND in inventory`);
            // Dump what we have
            const slots = bot.inventory.slots.filter((s) => s && s.count > 0).map((s, i) => `${i}:${s!.name}`);
        ...
        resolve({
          ...
          message: complete ? result.message : `${result.message} | isComplete: false`,
          inventory: inv,
        });
      } catch (err) {
        clearTimeout(timer);
...
const main = async () => {
  ...
  initLogger(dbPath);
  console.log(`Bench: ${step.name} (${stepId}) — ${count} bots, ${timeoutSec}s timeout`);
  ...
  console.log(`${"bot".padEnd(10)} ${"time".padEnd(8)} ${"result".padEnd(8)} message`);
  console.log(`${"─".repeat(70)}`);
  for (const r of allResults.sort((a, b) => a.name.localeCompare(b.name))) {
    const tag = r === winner ? "WINNER" : r.success ? "OK" : "FAIL";
    console.log(`${r.name.padEnd(10)} ${(r.elapsed + "s").padEnd(8)} ${tag.padEnd(8)} ${r.message}`);
  }
  console.log(`${"─".repeat(70)}`);
