  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      const b = await new Promise<Bot>((resolve, reject) => {
        log(`Connecting to ${HOST}:${PORT} as ${USERNAME} (attempt ${attempt})...`);
        const b = createMcBot({
        ...
        b.on("end", () => {
          ...
          else { log("Bot disconnected"); botReady = false; bot = null; }
        });
      ...
      log(`Bot ready at ${Math.floor(p.x)}, ${Math.floor(p.y)}, ${Math.floor(p.z)}`);
      ...
      b.on("kicked", (reason) => { log("Bot kicked:", reason); botReady = false; bot = null; });
...
const requireBot = (): Bot => {
  if (!bot || !botReady) throw new Error("Bot not connected. Is the MC server running?");
  return bot;
};
...
server.tool(
  ...
  async () => {
    const b = requireBot();
    const items = windowItems(b.inventory)
      .map((item, idx) => ({ slot: idx, name: item.name, count: item.count }));
  ...
  async ({ radius = 5 }) => {
    ...
    for (let dx = -r; dx <= r; dx++) {
      for (let dy = -r; dy <= r; dy++) {
        for (let dz = -r; dz <= r; dz++) {
          const block = b.blockAt({ x: Math.floor(pos.x) + dx, y: Math.floor(pos.y) + dy, z: Math.floor(pos.z) + dz } as any) as any;
    ...
    const entities = Object.values(b.entities)
      ...
      .map((e: any) => ({
        ...
        distance: +Math.sqrt((e.position.x - pos.x) ** 2 + (e.position.y - pos.y) ** 2 + (e.position.z - pos.z) ** 2).toFixed(1),
      }))
    ...
    return {
      content: [{
        ...
        text: JSON.stringify({
          position: { x: +pos.x.toFixed(1), y: +pos.y.toFixed(1), z: +pos.z.toFixed(1) },
          blockCounts: Object.entries(blocks).sort((a, b) => b[1] - a[1]).slice(0, 30),
...
Long-running operations block until complete (mining ~30-60s, navigation ~10s).`,
  {
    code: z.string().describe("TypeScript code to execute. Runs as: async (bot, state) => { YOUR_CODE }"),
    timeout: z.number().optional().describe("Timeout in ms (default 120000)"),
  },
  async ({ code, timeout = 120000 }) => {
    if (evalBusy) {
      return { content: [{ type: "text" as const, text: "ERROR: Another eval is already running" }], isError: true };
    }
    evalBusy = true;
    ...
    try {
      ...
      writeFileSync(evalTmpFile, `export default async function(bot: any, state: any) { ${code} }\n`);

      const mod = await import(`${evalTmpFile}?t=${Date.now()}`);
      const result = await Promise.race([
        mod.default(b, state),
        new Promise((_, reject) => setTimeout(() => reject(new Error(`Eval timed out (${timeout}ms)`)), timeout)),
      ]);

      const text = result === undefined ? "(no return value)" : JSON.stringify(result, null, 2);
      const p = b.entity.position;
      ...
      return { content: [{ type: "text" as const, text: `ERROR: ${msg}${stack}` }], isError: true };
    } finally {
      evalBusy = false;
    }
  },
