...
const bot: Bot = createBot({ host: HOST, port: PORT, username: USERNAME, version: VERSION, auth: "offline" });
bot.on("error", (e) => {
...
bot.once("spawn", async () => {
  ...
  console.log(`READY ${USERNAME} at ${Math.floor(p.x)}, ${Math.floor(p.y)}, ${Math.floor(p.z)}`);
  ...
  watchFile(CMD_FILE, { interval: 500 }, async () => {
    ...
    try {
      ...
      writeFileSync(tmpFile, `export default async function(bot: any) { ${cmd} }\n`);
