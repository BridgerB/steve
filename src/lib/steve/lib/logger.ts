...
let raceId: string = "";
let botId: string = "";
let diagBot: Bot | null = null; // set by attachDiagnostics — lets logEvent auto-capture pose
...
export const attachDiagnostics = (bot: Bot): void => {
	diagBot = bot; // so logEvent can auto-capture position + look on every event
