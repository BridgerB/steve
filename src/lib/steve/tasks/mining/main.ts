...
const branchMineOre = async (
	...
	while (have() < targetCount && Date.now() < deadline) {
		...
		// Advance ONE straight 1x2 tunnel at this level. A single clean tunnel, NOT a
		// grid of side-branches clearing out a whole room. digStep stays level (it
		// returns non-"ok" at caves/lava/water), so we turn rather than dropping or
		// flooding; mining the exposed walls is what surfaces ore as we go.
...
const mineDeepOre = async (
	...
	if (floorY(bot) > level + 2) {
		...
		logEvent(mineCat(blockType),"descended", `y=${res.y} stopped=${res.stopped}`);
		...
		logEvent(mineCat(blockType),"mine_in_place", `boxed at y=${res.y}, branch-mining here`);
	}
