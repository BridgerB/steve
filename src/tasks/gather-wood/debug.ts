...
bot.once("spawn", async () => {
	console.log(
		`Bot spawned at ${bot.entity.position.x}, ${bot.entity.position.y}, ${bot.entity.position.z}`,
	);
	...
	console.log(
		`Found log: ${log.name} at ${log.position.x}, ${log.position.y}, ${log.position.z}`,
	);
	...
	console.log(`\n=== BEFORE MOVING ===`);
	console.log(
		`Bot position: ${bot.entity.position.x}, ${bot.entity.position.y}, ${bot.entity.position.z}`,
	);
	...
	if (dist > 4) {
		...
		console.log(
			`New position: ${bot.entity.position.x}, ${bot.entity.position.y}, ${bot.entity.position.z}`,
		);
	}
	...
	if (cursorBlock?.position && equals(cursorBlock.position, targetPos)) {
		...
		console.log(
			`  Have: ${cursorBlock?.position?.x}, ${cursorBlock?.position?.y}, ${cursorBlock?.position?.z}`,
		);
	}
	...
	bot.on("itemDrop" as any, (entity: any) => {
		console.log(
			`ITEM DROP: ${entity.name} at ${entity.position?.x}, ${entity.position?.y}, ${entity.position?.z}`,
		);
	});
	...
	console.log(
		`Logs in inventory before: ${logsBefore.reduce((s, i) => s + i.count, 0)}`,
	);
	...
	console.log(
		`Now at: ${bot.entity.position.x}, ${bot.entity.position.y}, ${bot.entity.position.z}`,
	);
	console.log(
		`Distance to target: ${distance(bot.entity.position, targetPos).toFixed(
			2,
		)}`,
	);
	...
	console.log(
		`Logs in inventory after: ${logsAfter.reduce((s, i) => s + i.count, 0)}`,
	);
	...
	for (const item of items.slice(0, 3)) {
		...
		console.log(
			`  Item at ${item.position.x}, ${item.position.y}, ${item.position.z}, dist=${d.toFixed(2)}`,
		);
	}
