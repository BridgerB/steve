{
  ...
  outputs = {
    ...
    startServer = pkgs.writeShellScriptBin "minecraft-server" ''
      ...
      echo "Starting Minecraft Server (${pkgs.minecraft-server.version})..."
      echo "Server will be available at: localhost:25565"
      echo "Max players: 1000 | Online mode: OFF"
      echo ""

      exec ${pkgs.minecraft-server}/bin/minecraft-server ${jvmOpts}
    '';
  in {
    packages.${system}.default = startServer;
