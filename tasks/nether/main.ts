...
export const findFortress = async (bot: Bot): Promise<StepResult> => {
  ...
  let fortress = bot.findBlock({
    matching: (b) => fortressBlocks.includes(b.name),
    maxDistance: 64,
  });
  ...
  await bot.lookAt(bot.entity.position.offset(100, 0, 0));
...
export const killBlazes = async (
  ...
  while (rodsCollected < targetRods && attempts < maxAttempts) {
    ...
    if (blazes.length === 0) {
      ...
      const spawner = bot.findBlock({
        matching: (b) => b.name === "spawner",
        maxDistance: 32,
      });

      if (spawner) {
        // Move near spawner and wait
        const distance = bot.entity.position.distanceTo(spawner.position);
        if (distance > 8) {
          await bot.lookAt(spawner.position);
          bot.setControlState("forward", true);
          await new Promise((resolve) =>
            setTimeout(resolve, Math.min(distance * 100, 3000))
          );
          bot.setControlState("forward", false);
        }
    ...
    try {
      ...
      if (distance > 4) {
        ...
        bot.setControlState("forward", true);
        await new Promise((resolve) =>
          setTimeout(resolve, Math.min(distance * 80, 2000))
        );
        bot.setControlState("forward", false);
      }
  ...
  // Count actual rods
  const rods = bot.inventory.items().find((i) => i.name === "blaze_rod");
