...
export const smeltItems = async (
  ...
  if (!furnace) {
    ...
    const furnaceItem = bot.inventory.items().find(
      (i) => i.name === "furnace",
    );
  ...
  if (distance > 4) {
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, Math.min(distance * 150, 2000))
    );
  ...
  try {
    ...
    const itemsToSmelt = bot.inventory.items().filter(
      (i) => i.name.includes(inputItem),
    );
