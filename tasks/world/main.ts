...
export const fillWaterBucket = async (bot: Bot): Promise<StepResult> => {
  ...
  try {
    ...
    let distance = bot.entity.position.distanceTo(water.position);
    console.log(`  Distance to water: ${distance.toFixed(1)} blocks`);
    ...
    while (distance > 3) {
      ...
      const newDistance = bot.entity.position.distanceTo(water.position);
      console.log(`  Moving... distance: ${newDistance.toFixed(1)}`);

      // If we're not getting closer, we might be stuck
      if (newDistance >= distance - 0.5) {
        // Try jumping
    ...
    bot.setControlState("forward", false);
    bot.setControlState("sprint", false);
    console.log(`  Reached water, distance: ${distance.toFixed(1)}`);
    ...
    // In mineflayer, we need to use bot._genericPlace to fill bucket from water
    // Or we can try activateItem while looking at water
    ...
    let waterBucket = bot.inventory.items().find((i) =>
      i.name === "water_bucket"
    );

    if (!waterBucket) {
      // Try finding the block below/beside the water and activating that
      const belowWater = bot.blockAt(water.position.offset(0, -1, 0));
      if (
        ...
        await bot.lookAt(belowWater.position.offset(0.5, 1, 0.5)); // Look at top face
        await new Promise((resolve) => setTimeout(resolve, 200));
        try {
          // @ts-ignore - activateBlock with direction
          await bot.activateBlock(belowWater, { x: 0, y: 1, z: 0 });
          await new Promise((resolve) => setTimeout(resolve, 500));
        } catch {
      ...
      waterBucket = bot.inventory.items().find((i) =>
        i.name === "water_bucket"
      );
    }
...
export const buildNetherPortal = async (bot: Bot): Promise<StepResult> => {
  ...
  if (obsidianCount < 10) {
    ...
    const waterBucket = bot.inventory.items().find(
  ...
  try {
    ...
    const flintAndSteel = bot.inventory.items().find(
...
export const enterPortal = async (bot: Bot): Promise<StepResult> => {
  ...
  const portal = bot.findBlock({
    matching: (b) => b.name === "nether_portal" || b.name === "end_portal",
    maxDistance: 16,
  });
...
export const findStronghold = async (bot: Bot): Promise<StepResult> => {
  // Get eyes of ender
  const eyes = bot.inventory.items().find((i) => i.name === "ender_eye");
...
export const enterEndPortal = async (bot: Bot): Promise<StepResult> => {
  ...
  try {
    ...
    const _startDim = bot.game.dimension;
